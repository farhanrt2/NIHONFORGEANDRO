...
import { countKotoba, countKotobaByLevel, getKotobaForTest, saveHistory, recordWrongWord, getLastKotobaProgress, getDb } from "./db";
...
interface KotobaTestProps { onClose: () => void; }
type Mode = "JtoI" | "ItoJ";
type Method = "Reading" | "Listening";
type MemFilter = "all" | "memorized" | "not_memorized";

interface Q { jp: string; id: string; rowid: number; memorized: number; user: string; status: "" | "BENAR" | "SALAH"; }
interface SavedProgress { questions: Q[]; index: number; mode: Mode; method: Method; correct: number; wrong: number; startNo: number; endNo: number; }

const PROGRESS_KEY = "kotoba_progress";

export default function KotobaTest({ onClose }: KotobaTestProps) {
  const confirm = useConfirm();
  const alert = useAlert();

  const [maxItems, setMaxItems] = useState(0);
  const [mode, setMode] = useState<Mode>("JtoI");
  const [method, setMethod] = useState<Method>("Reading");
  ...
  useEffect(() => {
    (async () => {
      const total = await countKotoba();
      setMaxItems(total);
      const last = await getLastKotobaProgress();
      if (last) setLastProgress(`Progress terakhir: Kata ke-${last.endRange} (Skor: ${last.skor}%)`);
      else setLastProgress("Belum ada data Kotoba.");
      const db = await getDb();
      const rows = await db.select<{ value: string }[]>("SELECT value FROM settings WHERE key = $1", [PROGRESS_KEY]);
      if (rows.length > 0) {
        try {
          const p: SavedProgress = JSON.parse(rows[0].value);
          if (p.questions && p.index < p.questions.length) {
            setQuestions(p.questions); setIndex(p.index); setMode(p.mode); setMethod(p.method);
  ...
  async function onLevelChange(lvl: string) {
    setLevelFilter(lvl);
    const newMax = lvl === "all" ? await countKotoba() : await countKotobaByLevel(lvl);
    setMaxItems(newMax);
  ...
  async function startTest() {
    ...
    const ok = await confirm(`Mulai test ${filterLabel}${lvlTxt} kata ke-${sn} s/d ${en}?\nProgress batch ini akan dihapus.`);
    if (!ok) return;
  ...
  async function doLoad(filter: MemFilter) {
    ...
    setLoading(true);
    try {
      const raw = await getKotobaForTest(sn, en, filter, levelFilter);
      if (raw.length === 0) {
        const lvlTxt = levelFilter === "all" ? "" : ` level ${levelFilter}`;
        await alert(`Tidak ada kotoba${lvlTxt} di range ${sn}-${en}. Coba ubah range atau level!`);
        return;
      }
      if (raw.length < requested) {
        const lvlTxt = levelFilter === "all" ? "" : ` level ${levelFilter}`;
        await alert(`Hanya ${raw.length} kotoba${lvlTxt} ditemukan di range ${sn}-${en} (dari ${requested} yang diminta). Sisa akan dilompati.`);
      }
      const qs: Q[] = shuffle(raw).map(r => ({ jp: r.jp, id: r.id, rowid: r.rowid, memorized: r.memorized, user: "", status: "" }));
      setQuestions(qs); setIndex(0); setCorrect(0); setWrong(0);
      setActive(true); setFinished(false); setFeedback(null); setAnswered(false); setAnswer("");
      await saveProgress({ questions: qs, index: 0, mode, method, correct: 0, wrong: 0, startNo: sn, endNo: en });
      if (method === "Listening") setTimeout(() => playCurrent(qs[0]), 300);
    } finally {
      setLoading(false);
    }
  }

  function playCurrent(q?: Q) {
    const cur = q ?? questions[index];
    if (!cur) return;
    if (mode === "JtoI") speak(cur.jp, "ja"); else speak(cur.id, "id");
  }

  const current = questions[index];
  const soalText = current ? (mode === "JtoI" ? current.jp : current.id) : "";
  const pct = questions.length ? Math.round(((index + 1) / questions.length) * 100) : 0;

  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key !== "Enter" || e.shiftKey) return;
      // abaikan Enter dari IME (konfirmasi konversi → jangan hitung jawaban)
  ...
  }, [active, current, answered, answer, index, questions, correct, wrong, mode, method]);

  async function cek() {
    if (!current) return;
    if (answered) return; // guard dobel-hitung (Enter ganda cepat)
    if (!answer.trim()) { await alert("Jawaban tidak boleh kosong!"); return; }
    const target = mode === "JtoI" ? current.id : current.jp;
    const isCorrect = checkAnswer(answer, target);
    const newQ = [...questions];
    newQ[index] = { ...current, user: answer.trim(), status: isCorrect ? "BENAR" : "SALAH" };
    setQuestions(newQ);
    let nc = correct, nw = wrong;
    if (isCorrect) { nc++; setCorrect(nc); setFeedback({ text: "BENAR!", ok: true, jawaban: target }); }
    else { nw++; setWrong(nw); setFeedback({ text: "SALAH!", ok: false, jawaban: target }); await recordWrongWord(current.jp); }
    setAnswered(true);
    await saveProgress({ questions: newQ, index, mode, method, correct: nc, wrong: nw, startNo: parseInt(startNo, 10) || 1, endNo: parseInt(endNo, 10) || 1 });
  }
  ...
  async function next() {
    ...
    const ni = index + 1;
    setIndex(ni); setAnswer(""); setFeedback(null); setAnswered(false);
    await saveProgress({ questions, index: ni, mode, method, correct, wrong, startNo: parseInt(startNo, 10) || 1, endNo: parseInt(endNo, 10) || 1 });
    if (method === "Listening") setTimeout(() => playCurrent(questions[ni]), 300);
  }

  async function finish() {
    const total = questions.length;
    const skor = total > 0 ? Math.round((correct / total) * 100) : 0;
    await saveHistory(`${startNo}-${endNo}`, skor, mode);
    await clearProgress();
    setFinished(true);
  }
  ...
  async function resetProgress() {
    ...
    setFeedback(null); setAnswered(false); setFinished(false); setAnswer("");
  }
  ...
  async function switchMode(newMode: Mode) {
    if (active) {
      ...
      if (!ok) return;
      setIndex(0); setCorrect(0); setWrong(0); setFeedback(null); setAnswered(false); setAnswer("");
      await saveProgress({ questions, index: 0, mode: newMode, method, correct: 0, wrong: 0, startNo: parseInt(startNo, 10) || 1, endNo: parseInt(endNo, 10) || 1 });
    }
    setMode(newMode);
  }
  ...
  if (loading) return (
    <ScreenLayout subtitle="KOTOBA TEST" onClose={onClose} logo={80} logoCentered headerRight={headerRight} width={contentW} fillHeight>
  ...
  return (
    <ScreenLayout subtitle="KOTOBA TEST" onClose={onClose} logo={80} logoCentered headerRight={headerRight} width={contentW} fillHeight>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "10px 16px", background: "rgba(255,255,255,0.03)", borderRadius: 10, flexWrap: "wrap" }}>
        ...
        <button className="nf-btn nf-btn-neon" style={{"--btn-bg1":mode==="JtoI"?"#100520":"#1a1a2e","--btn-bg2":mode==="JtoI"?"#9a3dff":"#2a2a3e","--btn-neon1":mode==="JtoI"?"#b86aff":"#3a3a5e","--btn-neon2":mode==="JtoI"?"#9a3dff":"#2a2a3e","--btn-text":mode==="JtoI"?"#fff":"#666",fontSize:15,padding:"8px 18px"} as React.CSSProperties}
          onClick={() => switchMode("JtoI")}>Jepang → Indonesia</button>
        <button className="nf-btn nf-btn-neon" style={{"--btn-bg1":mode==="ItoJ"?"#100520":"#1a1a2e","--btn-bg2":mode==="ItoJ"?"#9a3dff":"#2a2a3e","--btn-neon1":mode==="ItoJ"?"#b86aff":"#3a3a5e","--btn-neon2":mode==="ItoJ"?"#9a3dff":"#2a2a3e","--btn-text":mode==="ItoJ"?"#fff":"#666",fontSize:15,padding:"8px 18px"} as React.CSSProperties}
          onClick={() => switchMode("ItoJ")}>Indonesia → Jepang</button>

        <div style={{ height: 24, width: 1, background: "rgba(255,255,255,0.1)" }} />

        <span className="text-gradient" style={{ fontSize: 16, fontWeight: 800, letterSpacing: 1 }}>METODE</span>
        <button className="nf-btn nf-btn-neon" style={{"--btn-bg1":method==="Reading"?"#001015":"#1a1a2e","--btn-bg2":method==="Reading"?"#1ac8c8":"#2a2a3e","--btn-neon1":method==="Reading"?"#3aebeb":"#3a3a5e","--btn-neon2":method==="Reading"?"#1ac8c8":"#2a2a3e","--btn-text":method==="Reading"?"#fff":"#666",fontSize:15,padding:"8px 18px"} as React.CSSProperties}
          onClick={() => setMethod("Reading")}>Reading</button>
        <button className="nf-btn nf-btn-neon" style={{"--btn-bg1":method==="Listening"?"#001015":"#1a1a2e","--btn-bg2":method==="Listening"?"#1ac8c8":"#2a2a3e","--btn-neon1":method==="Listening"?"#3aebeb":"#3a3a5e","--btn-neon2":method==="Listening"?"#1ac8c8":"#2a2a3e","--btn-text":method==="Listening"?"#fff":"#666",fontSize:15,padding:"8px 18px"} as React.CSSProperties}
          onClick={() => setMethod("Listening")}>Listening</button>
      </div>
      )}
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", padding: "10px 16px", background: "rgba(255,255,255,0.03)", borderRadius: 10, marginTop: 10 }}>
        ...
        <span style={{ fontSize: 13, color: "#4d94ff", textShadow: "0 0 8px rgba(77,148,255,0.3)" }}>
          {levelFilter === "all" ? `Semua: ${maxItems} kata` : `Level ${levelFilter}: ${maxItems} kata`}
        </span>
      </div>
      )}
      ...
      {!active && (
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, color: "#333" }}>
          <div className="jp" style={{ fontSize: 80, opacity: 0.1 }}>語</div>
      ...
      {/* Question area */}
      {active && current && (
        <div key={`${index}-${method}`} style={{ animation: "fadeCard 0.25s ease", display: "flex", flexDirection: "column", gap: 14, flex: 1, marginTop: 10 }}>
          {/* Label row */}
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div style={{ fontSize: 12, color: "#555" }}>{mode === "JtoI" ? "Jepang → Indonesia" : "Indonesia → Jepang"}</div>
            {method === "Listening" && (
              <button className="nf-btn nf-btn-neon" style={{"--btn-bg1":"#001015","--btn-bg2":"#1ac8c8","--btn-neon1":"#3aebeb","--btn-neon2":"#1ac8c8","--btn-text":"#fff",fontSize:13,padding:"4px 14px"} as React.CSSProperties}
                onClick={() => playCurrent()}>🔊 Putar Audio</button>
            )}
          </div>
          ...
          <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: 12, display: "flex", alignItems: "center", justifyContent: "center", minHeight: 100 }}>
            {method === "Listening" ? (
              <div style={{ textAlign: "center" }}>
                ...
                <div style={{ color: "#555", fontSize: 18 }}>Dengarkan baik-baik...</div>
                <button className="nf-btn nf-btn-neon" style={{"--btn-bg1":"#001015","--btn-bg2":"#1ac8c8","--btn-neon1":"#3aebeb","--btn-neon2":"#1ac8c8","--btn-text":"#fff",fontSize:15,padding:"8px 20px",marginTop:12} as React.CSSProperties}
                  onClick={() => playCurrent()}>🔊 PUTAR ULANG</button>
              </div>
            ) : (
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>
                  {mode === "JtoI" ? "Apa arti dari kata ini?" : "Bagaimana cara menulisnya dalam bahasa Jepang?"}
                </div>
                <div className="jp" style={{ fontSize: 42, color: "#fff", fontWeight: "bold", lineHeight: 1.1, textShadow: "0 0 20px rgba(255,255,255,0.1)" }}>{soalText}</div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <textarea key={index} value={answer} onChange={e => setAnswer(e.target.value)} disabled={answered} autoFocus
              placeholder="Ketik jawaban di sini... (Enter untuk cek / lanjut)"
...
function ResultScreen({ questions, onRetry, onClose }: { questions: Q[]; correct: number; wrong: number; onRetry: () => void; onClose: () => void; }) {
  ...
  async function bulkSetMemorized(ids: number[], value: number) {
    ...
    for (let i = 0; i < ids.length; i += CHUNK) {
      ...
      await db.execute(
        `UPDATE kotoba SET memorized = ${value} WHERE id IN (${part.map((_, j) => `$${j + 1}`).join(",")})`,
        part
      );
    }
  }
  ...
  async function handleHafal(rowid: number) {
    try {
      const db = await getDb();
      await db.execute("UPDATE kotoba SET memorized = 1 WHERE id = $1", [rowid]);
  ...
  async function handleUnhafal(rowid: number) {
    try {
      const db = await getDb();
      await db.execute("UPDATE kotoba SET memorized = 0 WHERE id = $1", [rowid]);
      setHafalSet(prev => { const s = new Set(prev); s.delete(rowid); return s; });
    } catch (e) { alert("Error: " + String(e)); }
  }

  return (
    <ScreenLayout subtitle="📊 HASIL TEST" onClose={onClose} logo={60} logoCentered width={1000} fillHeight
      headerRight={
      ...
      <div style={{ animation: "fadeCard 0.25s ease", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1.6fr 1.6fr 100px 130px", gap: 10, padding: "12px 16px", background: "rgba(255,255,255,0.05)", color: "#fff", fontWeight: "bold", fontSize: 12, textTransform: "uppercase", letterSpacing: 1 }}>
            <div>Soal (JP)</div><div>Arti (ID)</div><div>Jawaban Anda</div><div style={{ textAlign: "center" }}>Status</div><div style={{ textAlign: "center" }}>Aksi</div>
          </div>
            ...
            {shown.map((q, i) => {
              ...
              return (
                <div key={i} style={{ display: "grid", gridTemplateColumns: "1.2fr 1.6fr 1.6fr 100px 130px", gap: 10, padding: "10px 16px", alignItems: "center", background: i % 2 ? "rgba(255,255,255,0.03)" : "transparent", color: "#ddd", borderLeft: `4px solid ${benar ? "#3aa655" : "#c0392b"}` }}>
                  <div className="jp" style={{ fontSize: 18 }}>{q.jp}</div>
                  <div>{q.id}</div>
                  <div style={{ fontStyle: q.user ? "normal" : "italic", color: q.user ? "#ddd" : "#666" }}>{q.user || "(kosong)"}</div>
                  <div style={{ textAlign: "center" }}>
